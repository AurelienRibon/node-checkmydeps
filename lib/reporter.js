...
exports.generateSingleRepoReport = function(dependencies, options = {}) {
  const report = generateReport(dependencies, options);
  return formatReport(report).trim();
};

exports.generateMultiRepoReport = function(dependenciesByModule, addSeparators, options = {}) {
  let fullReport = '';

  const moduleNames = Object.keys(dependenciesByModule).sort();

  for (const moduleName of moduleNames) {
    const dependencies = dependenciesByModule[moduleName];

    if (dependencies.length > 0) {
      const report = generateReport(dependencies, options);
      const separator = addSeparators ? getSeparator(moduleName) : '';

      fullReport += separator + report;
    }
  }
...
function getSeparator(moduleName) {
  return '\n\n> ' + moduleName + '\n' + '%' + '\n';
}
